...
class AudioPlayService : BaseService(), AudioManager.OnAudioFocusChangeListener, Player.Listener {
    ...
    override fun onDestroy(){if(!pause)AudioPlay.upReadTime();super.onDestroy();if(useWakeLock){wakeLock.release();wifiLock?.release()};isRun=false;bufferedPosition=0;abandonFocus();exoPlayer.release();mediaSessionCompat.release();broadcastReceiver?.let{unregisterReceiver(it)};upMediaSessionPlaybackState(PlaybackStateCompat.STATE_STOPPED);AudioPlay.status=Status.STOP;postEvent(EventBus.AUDIO_STATE,Status.STOP);AudioPlay.unregisterService();upNotificationJob?.invokeOnCompletion{notificationManager.cancel(NotificationId.AudioPlayService)}}
    @OptIn(UnstableApi::class) @SuppressLint("WakelockTimeout") private fun play(){if(useWakeLock){wakeLock.acquire();wifiLock?.acquire()};upAudioPlayNotification();if(!requestFocus()){if(useWakeLock){wakeLock.release();wifiLock?.release()};return};val book=AudioPlay.book;execute(context=Main){AudioPlay.status=Status.STOP;postEvent(EventBus.AUDIO_STATE,Status.STOP);upPlayProgressJob?.cancel();if(url.isJsonArray()){val mediaSource=ExoPlayerHelper.getMediaSource(this@AudioPlayService,url);if(mediaSource==null){NoStackTraceException("url格式错误");return@execute};exoPlayer.setMediaSource(mediaSource);position=0}else{val analyzeUrl=AnalyzeUrl(url,source=AudioPlay.bookSource,ruleData=book,chapter=AudioPlay.durChapter,coroutineContext=coroutineContext);exoPlayer.setMediaItem(analyzeUrl.getMediaItem())};exoPlayer.playWhenReady=true;val skipStartMs=(book?.getOpenCredits()?:0)*1000L;exoPlayer.seekTo(if(position==0)skipStartMs else position.toLong());exoPlayer.prepare()}.onError{AppLog.put("播放出错\n${it.localizedMessage}",it);toastOnUi("$url ${it.localizedMessage}");MaxAudioSystem.setError(it.localizedMessage);stopSelf()}}
    ...
    @SuppressLint("WakelockTimeout") private fun resume(){if(useWakeLock){wakeLock.acquire();wifiLock?.acquire()};if(!hasAudioFocus&&!requestFocus()){if(useWakeLock){wakeLock.release();wifiLock?.release()};return};try{AudioPlay.markReadStart();pause=false;reassertMediaSession();if(url.isEmpty()){AudioPlay.loadOrUpPlayUrl();return};if(exoPlayer.playbackState==Player.STATE_IDLE){position=AudioPlay.durChapterPos;play();return};if(!exoPlayer.isPlaying)exoPlayer.play();upPlayProgress();AudioPlay.status=Status.PLAY;postEvent(EventBus.AUDIO_STATE,Status.PLAY);upAudioPlayNotification()}catch(e:Exception){e.printOnDebug();MaxAudioSystem.setError(e.localizedMessage);stopSelf()}}
    ...
    override fun onAudioFocusChange(focusChange:Int){if(AppConfig.ignoreAudioFocus)return;when(focusChange){AudioManager.AUDIOFOCUS_GAIN->{hasAudioFocus=true;if(needResumeOnAudioFocusGain){needResumeOnAudioFocusGain=false;resume()}};AudioManager.AUDIOFOCUS_LOSS->pause();AudioManager.AUDIOFOCUS_LOSS_TRANSIENT->{if(!pause){needResumeOnAudioFocusGain=true;pause(false)}};AudioManager.AUDIOFOCUS_LOSS_TRANSIENT_CAN_DUCK->{}}}
    ...
    private fun requestFocus():Boolean{if(AppConfig.ignoreAudioFocus){hasAudioFocus=true;return true};hasAudioFocus=MediaHelp.requestFocus(mFocusRequest);return hasAudioFocus}
    private fun abandonFocus(){hasAudioFocus=false;@Suppress("DEPRECATION") audioManager.abandonAudioFocus(this)}
    /**
     * 重新激活媒体会话，夺回媒体按键（蓝牙/耳机播放暂停）的路由。
     * 其他应用播放过音频后，系统会把媒体按键交给最近激活的会话，
     * 而本会话全程处于激活状态，直接再置 true 不会改变优先级，
     * 需要先置 false 再置 true 触发系统重新注册。
     */
    private fun reassertMediaSession(){if(mediaSessionCompat.isActive)mediaSessionCompat.isActive=false;mediaSessionCompat.isActive=true}
}
