...
class AudioPlayService : BaseService(), AudioManager.OnAudioFocusChangeListener, Player.Listener {
    ...
    override fun onCreate(){super.onCreate();instance=this;isRun=true;bufferedPosition=0;exoPlayer.addListener(this);AudioPlay.registerService(this);initMediaSession();initBroadcastReceiver();upMediaSessionPlaybackState(PlaybackStateCompat.STATE_PAUSED);doDs();execute{ImageLoader.loadBitmap(this@AudioPlayService,AudioPlay.book?.let{BookCover.getDisplayCover(it)}).submit().get()}.onSuccess{if(it.width>16&&it.height>16){cover=it;upMediaMetadata();upAudioPlayNotification()}}}
    override fun onStartCommand(intent:Intent?,flags:Int,startId:Int):Int{when(intent?.action){IntentAction.play,IntentAction.playNew->{exoPlayer.stop();upPlayProgressJob?.cancel();pause=false;AudioPlay.markReadStart();position=if(intent.action==IntentAction.playNew)0 else AudioPlay.book?.durChapterPos?:0;url=AudioPlay.durPlayUrl;if(playSpeed!=1f)upSpeed(playSpeed);upMediaSessionPlaybackState(PlaybackStateCompat.STATE_BUFFERING);play()};IntentAction.stopPlay->{AudioPlay.upReadTime();exoPlayer.stop();upPlayProgressJob?.cancel();AudioPlay.status=Status.STOP;upMediaSessionPlaybackState(PlaybackStateCompat.STATE_STOPPED);postEvent(EventBus.AUDIO_STATE,Status.STOP);pause=true;upAudioPlayNotification()};IntentAction.pause->pause();IntentAction.resume->resume();IntentAction.prev->MaxAudioSystem.previous();IntentAction.next->MaxAudioSystem.next();IntentAction.setSpeed->upSpeed(intent.getFloatExtra("speed",1f));IntentAction.addTimer->addTimer();IntentAction.setTimer->setTimer(intent.getIntExtra("minute",0));IntentAction.adjustProgress->adjustProgress(intent.getIntExtra("position",position));IntentAction.stop->{saveCurrentPosition();AudioPlay.upReadTime();pause=true;stopSelf()}};return super.onStartCommand(intent,flags,startId)}
    override fun onDestroy(){saveCurrentPosition();if(!pause)AudioPlay.upReadTime();super.onDestroy();if(useWakeLock){wakeLock.release();wifiLock?.release()};isRun=false;bufferedPosition=0;abandonFocus();exoPlayer.release();mediaSessionCompat.release();broadcastReceiver?.let{runCatching{unregisterReceiver(it)}};upMediaSessionPlaybackState(PlaybackStateCompat.STATE_STOPPED);AudioPlay.status=Status.STOP;postEvent(EventBus.AUDIO_STATE,Status.STOP);AudioPlay.unregisterService();instance=null;upNotificationJob?.invokeOnCompletion{notificationManager.cancel(NotificationId.AudioPlayService)}}
    private fun saveCurrentPosition(){runCatching{val current=exoPlayer.currentPosition.toInt().coerceAtLeast(0);position=current;AudioPlay.durChapterPos=current;AudioPlay.forceSaveRead()}}
    fun refreshCurrentMediaInfo(){upMediaMetadata();upAudioPlayNotification()}
    ...
    private fun adjustProgress(position:Int){this.position=position.coerceAtLeast(0);AudioPlay.durChapterPos=this.position;AudioPlay.forceSaveRead();exoPlayer.seekTo(this.position.toLong())}
