...
class BooksAdapterList(context: Context, callBack: CallBack) :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when (viewType) {
            ...
            else -> if (AppConfig.bookLayout == 0) {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int, payloads: MutableList<Any>) {
        when (holder) {
            is BookViewHolder -> (getItem(position) as? Book)?.let { holder.registerListener(it); holder.onBind(it, position, payloads) }
            is BookViewHolder2 -> (getItem(position) as? Book)?.let { holder.registerListener(it); holder.onBind(it, position, payloads) }
            is GroupViewHolder -> (getItem(position) as? BookGroup)?.let { holder.registerListener(it); holder.onBind(it, position, payloads) }
            is GroupGridViewHolder -> (getItem(position) as? BookGroup)?.let { holder.registerListener(it); holder.onBind(it, position, payloads) }
        }
    }
    ...
    private fun bindAudioPlayButton(button: android.widget.ImageButton, item: Book) {
        ...
        button.setOnClickListener {
            ...
            refreshAudioButtonLater(button, 220L)
            refreshAudioButtonLater(button, 650L)
        }
    }
    ...
    inner class BookViewHolder(val binding: ItemBookshelfListBinding) : RecyclerView.ViewHolder(binding.root) {
        fun onBind(item: Book, position: Int) = binding.run {
            if (AppConfig.showBookBorder) {
                ...
                (root.layoutParams as? ViewGroup.MarginLayoutParams)?.setMargins(4.dpToPx(), 4.dpToPx(), 4.dpToPx(), 4.dpToPx())
            } else {
        ...
        private fun upRefresh(binding: ItemBookshelfListBinding, item: Book) {
            if (!item.isLocal && callBack.isUpdate(item.bookUrl)) { binding.bvUnread.invisible(); binding.rlLoading.visible() }
            else { binding.rlLoading.gone(); if (AppConfig.showUnread) { binding.bvUnread.setHighlight(item.lastCheckCount > 0); binding.bvUnread.setBadgeCount(item.getUnreadChapterNum()) } else binding.bvUnread.invisible() }
        }
        private fun upMoreInfo(binding: ItemBookshelfListBinding, item: Book) {
            if (AppConfig.showMoreInfoInList && AppConfig.showTagsInList) { binding.flexboxTags.visible(); updateTagViews(binding.flexboxTags, item) } else binding.flexboxTags.gone()
            if (AppConfig.showMoreInfoInList && AppConfig.showIntroInList) { binding.tvIntro.visible(); binding.tvIntro.text = item.getDisplayIntroPlainText(); binding.tvIntro.maxLines = AppConfig.introLinesInList } else binding.tvIntro.gone()
        }
        private fun updateTagViews(flexboxLayout: FlexboxLayout, item: Book) {
            flexboxLayout.removeAllViews()
            if (item.wordCount?.isNotBlank() == true) flexboxLayout.addView(createTagView(item.wordCount!!))
            val tagsText = item.customTag ?: item.kind ?: ""
            if (tagsText.isNotBlank()) for (tag in tagsText.splitNotBlank(",", "\n")) flexboxLayout.addView(createTagView(tag))
        }
        ...
        private fun upLastUpdateTime(binding: ItemBookshelfListBinding, item: Book) {
            if (AppConfig.showLastUpdateTime && !item.isLocal) { val time = item.latestChapterTime.toTimeAgo(); if (binding.tvLastUpdateTime.text != time) binding.tvLastUpdateTime.text = time }
    ...
    inner class BookViewHolder2(val binding: ItemBookshelfList2Binding) : RecyclerView.ViewHolder(binding.root) {
        ...
        fun registerListener(item: Any) { binding.root.setOnClickListener { callBack.onItemClick(item) }; binding.root.onLongClick { callBack.onItemLongClick(item) } }
    ...
    inner class GroupViewHolder(val binding: ItemBookshelfListGroupBinding) : RecyclerView.ViewHolder(binding.root) {
        fun onBind(item: BookGroup, position: Int) = binding.run { tvName.text = item.groupName; ivCover.load(item); flHasNew.gone(); ivAuthor.gone(); ivLast.gone(); ivRead.gone(); tvAuthor.gone(); tvLast.gone(); tvRead.gone() }
        fun onBind(item: BookGroup, position: Int, payloads: MutableList<Any>) = binding.run { if (payloads.isEmpty()) onBind(item, position) else { for (i in payloads.indices) { val bundle = payloads[i] as Bundle; bundle.keySet().forEach { when (it) { "groupName" -> tvName.text = item.groupName; "cover" -> ivCover.load(item) } } } } }
        fun registerListener(item: Any) { binding.root.setOnClickListener { callBack.onItemClick(item) }; binding.root.onLongClick { callBack.onItemLongClick(item) } }
    }

    inner class GroupGridViewHolder(val binding: ItemBookshelfGridGroupBinding) : RecyclerView.ViewHolder(binding.root) {
        fun onBind(item: BookGroup, position: Int) = binding.run { tvName.text = item.groupName; ivCover.load(item) }
        fun onBind(item: BookGroup, position: Int, payloads: MutableList<Any>) = binding.run { if (payloads.isEmpty()) onBind(item, position) else { for (i in payloads.indices) { val bundle = payloads[i] as Bundle; bundle.keySet().forEach { when (it) { "groupName" -> tvName.text = item.groupName; "cover" -> ivCover.load(item) } } } } }
        fun registerListener(item: Any) { binding.root.setOnClickListener { callBack.onItemClick(item) }; binding.root.onLongClick { callBack.onItemLongClick(item) } }
    }
}