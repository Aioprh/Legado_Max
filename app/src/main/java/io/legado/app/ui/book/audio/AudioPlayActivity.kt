...
class AudioPlayActivity :
    ...
    override fun upLyric(lyric: String?) { if (oldLyric == lyric) return; oldLyric = lyric; binding.lyricViewX.animate().cancel(); binding.lyricViewX.animate().alpha(0f).translationY(10.dpToPx().toFloat()).setDuration(140).withEndAction { if (lyric.isNullOrBlank()) { binding.lyricViewX.gone(); return@withEndAction }; lyricViewX.loadLyric(lyric); binding.lyricViewX.visible(); binding.lyricViewX.animate().alpha(1f).translationY(0f).setDuration(280).start(); if (lyricOn) upLyricP(AudioPlay.durChapterPos) else { lyricOn = true; lyricViewX.apply { setNormalTextSize(46F); setCurrentTextSize(56F); setTimelineTextColor(accentColor); setDraggable(true, object : OnPlayClickListener { override fun onPlayClick(time: Long): Boolean { AudioPlay.adjustProgress(time.toInt()); playButton(false); return true } }) }; lyricViewX.postDelayed({ upLyricP(AudioPlay.durChapterPos) }, 100) } }.start() }
