...
object AiSourceValidate {
    ...
    fun validate(text: String): List<Check> {
        ...
        if (sourceUrl != null) {
            check("源地址格式", sourceUrl.startsWith("http://") || sourceUrl.startsWith("https://") || sourceUrl.startsWith("data:"), "bookSourceUrl 应以 http://、https:// 或 data: 开头")
        }
        val typeVal = src.get("bookSourceType")?.takeIf { it.isJsonPrimitive && it.asJsonPrimitive.isNumber }?.asJsonPrimitive?.asInt
        ...
        listOf("ruleSearch" to rs, "ruleBookInfo" to rb, "ruleToc" to rt, "ruleContent" to rc, "ruleExplore" to re).forEach { (section, obj) ->
            obj?.entrySet()?.forEach { (key, value) ->
                if (value.isJsonPrimitive && value.asJsonPrimitive.isString && value.asString.isNotBlank()) allRules.add("$section.$key" to value.asString)
            }
        }
        src.entrySet().forEach { (key, value) ->
            if (value.isJsonPrimitive && value.asJsonPrimitive.isString && value.asString.isNotBlank()) allRules.add("top.$key" to value.asString)
        }
        ...
        check("正则成对", oddRules.isEmpty(), if (oddRules.isNotEmpty()) "以下规则 ## 未成对：${oddRules.take(3).joinToString("、") { it.first }}" else "所有 ## 正则均已成对")
        ...
        check("正则语法", invalidRegex.isEmpty(), if (invalidRegex.isNotEmpty()) "以下规则正则无法编译：${invalidRegex.take(3).joinToString("、") { it.first }}" else "所有 ## 正则均可编译")

        val unbalancedBraces = allRules.filter { (_, r) -> r.count { it == '{' } != r.count { it == '}' } }
        check("花括号配对", unbalancedBraces.isEmpty(), if (unbalancedBraces.isNotEmpty()) "以下规则 { } 未配对：${unbalancedBraces.take(3).joinToString("、") { it.first }}" else "所有 { } 均配对")

        val unsupportedIds = findUnsupportedBookId(src)
        check("Legado 变量", unsupportedIds.isEmpty(), if (unsupportedIds.isNotEmpty()) "发现不存在的 bookId/chapterId 变量：${unsupportedIds.take(3).joinToString("、") { it.first }}；请从 book.bookUrl/chapter.url 或 JSONPath 获取 ID" else "未发现裸 bookId/chapterId")
        ...
        check("match 取下标安全", unsafeMatchRules.isEmpty(), if (unsafeMatchRules.isNotEmpty()) "${unsafeMatchRules.take(3).joinToString("、") { it.first }} 使用了 match()[n]；请改成 (xxx.match(/re/)||[])[n]||''" else "所有规则未直接对 match() 结果取下标")
        ...
        check("选择器稳定性", selectorHits.isEmpty(), if (selectorHits.isNotEmpty()) "发现高风险选择器 ${selectorHits.joinToString("、")}；优先使用稳定 class/id/属性选择器" else "未发现高风险选择器")

        val jsonParseThis = allRules.filter { it.second.contains("@js:JSON.parse(this)", ignoreCase = true) }
        check("JSON 列表规则", jsonParseThis.isEmpty(), if (jsonParseThis.isNotEmpty()) "${jsonParseThis.take(3).joinToString("、") { it.first }} 不能用 @js:JSON.parse(this) 解析整段响应，请改为 JSONPath" else "未发现 JSON.parse(this) 高风险写法")

        val emptyUrlRules = allRules.filter { hasEmptyUrlParameter(it.second) }
        check("URL 参数完整", emptyUrlRules.isEmpty(), if (emptyUrlRules.isNotEmpty()) "以下规则可能产生空 URL 参数：${emptyUrlRules.take(3).joinToString("、") { it.first }}" else "未发现明显空 URL 参数")

        val chapterUrl = strOf(rt, "chapterUrl").orEmpty()
        val tocUrl = strOf(rb, "tocUrl").orEmpty()
        check("目录-正文链路", chapterUrl.isBlank() || tocUrl.isBlank() || normalizeRule(chapterUrl) != normalizeRule(tocUrl), if (chapterUrl.isNotBlank() && tocUrl.isNotBlank() && normalizeRule(chapterUrl) == normalizeRule(tocUrl)) "ruleToc.chapterUrl 与 ruleBookInfo.tocUrl 完全相同，疑似把目录 URL 当正文 URL" else "chapterUrl 与 tocUrl 未发现静态重复")

        val relevant = AiSourceKnowledge.relevantRepairHints(text)
        check("知识规则命中", true, if (relevant.isEmpty()) "未命中额外高风险规则" else "命中：${relevant.joinToString("、") { it.id }}")
        return list
    }

    /**
     * 把静态检查结果转换为新的结构化问题模型。
     * 失败检查不再只能依赖中文文本，自动修复层可据 code/stage 选择不同修复策略。
     */
    fun structuredIssues(text: String): List<AiSourceValidationIssue> {
        val checks = validate(text)
        return checks.filter { !it.pass }.map { check ->
            val (stage, code, hint) = when {
                check.name.contains("JSON") || check.name.contains("结构") -> Triple(AiSourceValidationIssue.Stage.JSON, AiSourceValidationIssue.Code.INVALID_JSON, "先修复 JSON 结构，只输出合法的书源 JSON 数组。")
                check.name.contains("搜索") -> Triple(AiSourceValidationIssue.Stage.SEARCH, AiSourceValidationIssue.Code.SEARCH_FAILED, "检查 searchUrl、bookList、name、bookUrl，并依据真实搜索响应重写规则。")
                check.name.contains("详情") -> Triple(AiSourceValidationIssue.Stage.BOOK_INFO, AiSourceValidationIssue.Code.BOOK_INFO_FAILED, "依据真实详情页响应重写 ruleBookInfo，并确保 tocUrl 指向真实目录。")
                check.name.contains("目录") || check.name.contains("章节") -> Triple(AiSourceValidationIssue.Stage.TOC, AiSourceValidationIssue.Code.TOC_FAILED, "依据真实目录响应重写 chapterList/chapterName/chapterUrl，并验证多个章节 URL。")
                check.name.contains("正文") -> Triple(AiSourceValidationIssue.Stage.CONTENT, AiSourceValidationIssue.Code.CONTENT_FAILED, "依据真实正文响应重写 ruleContent.content。")
                check.name.contains("发现") -> Triple(AiSourceValidationIssue.Stage.EXPLORE, AiSourceValidationIssue.Code.EXPLORE_FAILED, "依据真实分类页响应重写 ruleExplore。")
                check.name.contains("段评") -> Triple(AiSourceValidationIssue.Stage.REVIEW, AiSourceValidationIssue.Code.REVIEW_RULE_MISSING, "仅在真实探测到段评接口时生成段评规则，不要虚构接口。")
                check.name.contains("URL") -> Triple(AiSourceValidationIssue.Stage.CONTENT, AiSourceValidationIssue.Code.INVALID_CHAPTER_URL, "所有详情、目录、正文 URL 必须最终解析为完整 http/https 地址。")
                else -> Triple(AiSourceValidationIssue.Stage.JSON, AiSourceValidationIssue.Code.INVALID_JSON, "${check.name}：${check.msg}")
            }
            AiSourceValidationIssue(code, stage, "${check.name}: ${check.msg}", hint)
        }
    }

    private fun normalizeRule(value: String): String = value.trim().replace("{{book.tocUrl}}", "").replace("{{book.bookUrl}}", "")

    private fun hasEmptyUrlParameter(value: String): Boolean = Regex("[?&][A-Za-z0-9_.-]+=(?:&|$)").containsMatchIn(value)
    ...
    fun findUnsupportedBookId(src: JsonObject): List<Pair<String, String>> {
        ...
        listOf("ruleSearch", "ruleBookInfo", "ruleToc", "ruleContent", "ruleExplore").forEach { sec -> src.getAsJsonObject(sec)?.let { scan(sec, it) } }
    ...
    fun toSourceJson(source: BookSource, original: JsonObject?): String {
        ...
        original?.let { orig -> listOf("header", "loginUrl", "loginCheckJs", "charset").forEach { k -> if (!obj.has(k)) orig.get(k)?.let { obj.add(k, it) } } }
        val arr = JsonArray()
        arr.add(obj)
        return arr.toString()
    }
