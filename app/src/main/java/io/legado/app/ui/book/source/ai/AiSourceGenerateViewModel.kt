    ...
    suspend fun autoFix(
        ...
        while (round < maxRounds) {
            ...
            val fixUserPrompt = buildString {
                ...
                if (reviewProbe) {
                    appendLine()
                    appendLine("【段评探测（已开启）】修复时必须保留并生成“段评气泡”规则（若上一轮被移除请恢复，不要因验证通过而删除）：正文规则 content 用 @js: 请求段评统计接口取每段段评数，把正文按换行拆段后，对段评数>0 的段落末尾插入 <img src=\"dp:<段评数>,{\\\"pclick\\\":\\\"<点击JS>\\\",\\\"status\\\":\\\"normal\\\"}\"> 气泡标记，点击气泡弹出该段段评内容（完整实现见系统提示词【段评气泡】模板，HOST 与接口路径替换为本站实际值）。")
