...
class AiSourceGenerateViewModel(application: Application) : BaseViewModel(application) {
    ...
    /** 是否在生成书源时探测并生成段评气泡规则（默认关闭） */
    var reviewProbe: Boolean
        get() = LocalConfig.getBoolean(KEY_REVIEW_PROBE, false)
        set(value) {
            LocalConfig.putBoolean(KEY_REVIEW_PROBE, value)
        }
    ...
    suspend fun autoFix(
        ...
        while (round < maxRounds) {
            ...
            val fixUserPrompt = buildString {
                ...
                if (reviewProbe) {
                    appendLine()
                    appendLine("【段评探测（已开启）】修复时必须保留并生成“段评气泡”规则（若上一轮被移除请恢复，不要因验证通过而删除）：正文规则 content 用 @js: 请求段评统计接口取每段段评数，把正文按换行拆段后，对段评数>0 的段落末尾插入 <img src=\"dp:<段评数>,{\\\"pclick\\\":\\\"<点击JS>\\\",\\\"status\\\":\\\"normal\\\"}\"> 气泡标记，点击气泡调用 java.showParagraphComments(JSON.stringify(config)) 打开 App 原生段评弹窗（展示头像/昵称/内容/点赞/时间，支持分页加载与回复展开；完整实现见系统提示词【段评气泡】模板，HOST 与接口路径替换为本站实际值）。")
                    if (reviewUrl.isNotBlank()) {
                        appendLine("段评接口：$reviewUrl（据此推断 summary/paragraph 等 action 参数与返回结构；type 参数用 type=text）")
                    }
                }
                appendLine()
    ...
    companion object {
        ...
        private const val KEY_REVIEW_PROBE = "ai_review_probe"
...
- 【段评气泡（可选）】若站点有“本章说/段评”接口（页面脚本出现 comments.php / comment.php / review.php，或用户开启段评探测并提供了段评接口），可在正文规则 content 里用 @js: 生成段评气泡：① 请求段评统计接口取每段段评数（起点系 comments.php?action=summary&book_id=...&chapter_id=... 返回 $.Data.Getparagraphscommentcounts.DataList[]，字段 ParagraphId/CommentCount，其中 ParagraphId=-1 为章评、不用于正文段落）；② 把正文按换行拆段（跳过空段），第 i 段对应 ParagraphId=i；③ 对段评数>0 的段落末尾插入气泡标记 <img src="dp:<段评数>,{\"pclick\":\"<点击JS>\",\"status\":\"normal\"}">；④ 点击 JS（pclick）调用 java.showParagraphComments(JSON.stringify(config)) 打开 App 原生段评弹窗（展示头像/昵称/等级/地区/时间/内容/赞踩/楼层，支持分页加载与回复展开）。config 为 JS 对象，字段：commentsUrl（段评列表 URL，分页用占位符 page=[page]、page_size=[pageSize]）、repliesUrl（回复列表 URL，评论 id 用 [reviewId]、根评论 id 用 [rootId]，站点无回复接口则省略该字段）、listPath/totalPath（列表/总数 JSONPath，如 $.Data.DataList / $.Data.TotalCount）、pageSize、fields（评论字段 JSONPath：nickname/avatar/level/ip/content/agree/oppose/time/floor/id/rootId/replyCount）、replyFields（回复字段 JSONPath：nickname/avatar/replyTo/content/agree/time）。起点系段评接口 comments.php?action=paragraph&book_id=...&chapter_id=...&paragraph_id=<段号>&type=text&page=1&page_size=20 返回 $.Data.DataList[]（type 参数优先用 type=text：部分起点系镜像站 type=all 会返回 “Service request failed”，仅 text/image 有效），字段 Content/Floor/AgreeAmount/CreateTime/UserName/UserHeadIcon/ShowTag/IpLocation/ReviewCount/Id/RootReviewId；回复接口 comments.php?action=replies&review_id=<评论id>&root_review_id=<根评论id> 返回同结构（回复对象含 RelatedUser/RelatedUserId）。【重要】pclick 代码里禁止出现双引号、反斜杠和尖括号 >（否则会破坏 img 的 src 属性、被 <img> 正则截断、或让 option 变成非法 JSON），字符串一律用单引号；commentsUrl/repliesUrl 里的 [page]/[pageSize]/[reviewId]/[rootId] 占位符必须原样保留在字符串中（由 App 运行时替换，注意不能写成 {{...}}，否则会被规则引擎当模板求值报错）。起点系参考模板（HOST 与接口路径按实际站点替换；嵌入 JSON content 字段时反斜杠按 JSON 转义）：
@js:(function(){var B=(book.bookUrl.split('book_id=')[1]||'').split('&')[0];var C=(chapter.url.split('chapter_id=')[1]||'').split('&')[0];var H='https://HOST';var txt='';try{var o=JSON.parse(java.ajax(H+'/content.php?book_id='+B+'&chapter_id='+C));txt=o.Content||''}catch(e){}if(txt){var cnt={};try{var s=JSON.parse(java.ajax(H+'/comments.php?action=summary&book_id='+B+'&chapter_id='+C));(s.Data.Getparagraphscommentcounts.DataList||[]).forEach(function(r){cnt[r.ParagraphId]=r.CommentCount})}catch(e){}var ps=txt.replace(/\r\n/g,'\n').split('\n'),pid=0,out=[];for(var i=0;i<ps.length;i++){var p=ps[i];if(!p.replace(/\s/g,'')){out.push(p);continue}pid++;var cc=cnt[pid]||0;if(cc>0){var pc='var B=(book.bookUrl.split(\'book_id=\')[1]||\'\').split(\'&\')[0];var C=(chapter.url.split(\'chapter_id=\')[1]||\'\').split(\'&\')[0];var cfg={listPath:\'$.Data.DataList\',totalPath:\'$.Data.TotalCount\',commentsUrl:\'https://HOST/comments.php?action=paragraph&book_id=\'+B+\'&chapter_id=\'+C+\'&paragraph_id='+pid+'&type=text&page=[page]&page_size=[pageSize]\',pageSize:20,repliesUrl:\'https://HOST/comments.php?action=replies&book_id=\'+B+\'&chapter_id=\'+C+\'&review_id=[reviewId]&root_review_id=[rootId]&page=1&page_size=[pageSize]\',fields:{nickname:\'$.UserName\',avatar:\'$.UserHeadIcon\',level:\'$.ShowTag\',ip:\'$.IpLocation\',content:\'$.Content\',agree:\'$.AgreeAmount\',oppose:\'$.OpposeAmount\',time:\'$.CreateTime\',floor:\'$.Floor\',id:\'$.Id\',rootId:\'$.RootReviewId\',replyCount:\'$.ReviewCount\'},replyFields:{nickname:\'$.UserName\',avatar:\'$.UserHeadIcon\',replyTo:\'$.RelatedUser\',content:\'$.Content\',agree:\'$.AgreeAmount\',time:\'$.CreateTime\'}};java.showParagraphComments(JSON.stringify(cfg))';out.push(p+'<img src="dp:'+cc+',{\\"pclick\\":\\"'+pc+'\\",\\"status\\":\\"normal\\"}">')}else{out.push(p)}}txt=out.join('\n')}return txt;})()
