...
object AudioPlay : CoroutineScope by MainScope() {
    ...
    /** 书架极小化 Book 缺少 durChapterPos，需用数据库完整图书重置，否则续播会从本集开头播放 */
    fun resetFromShelf(book: Book){val target=appDb.bookDao.getBook(book.bookUrl)?:book;resetData(target)}
    private fun loadPlayUrl(emptyRetry: Int = 0){
        val index = durChapterIndex
        val key = "${book?.bookUrl}#$index"
        synchronized(preloadedUrls){preloadedUrls.remove(key)}?.let{
            upDurChapter()
            durPlayUrl = it
        ...
        if(currentBook != null && source != null){
            ...
            WebBook.getContent(this,source,currentBook,chapter)
                .onSuccess{content->
                    ...
                    if(value.isEmpty()){
                        if(emptyRetry < 1){
                            AppLog.put("播放资源为空，自动重新获取一次")
                            removeLoading(index)
                            Coroutine.async {
                                kotlinx.coroutines.delay(150)
                                if(durChapterIndex == index) loadPlayUrl(emptyRetry + 1)
                            }
                        }else{
                            appCtx.toastOnUi("未获取到资源链接")
                        }
                    }else{
                        contentLoadFinish(chapter,value)
                    }
                }
                .onError{error->
                    if(error is ContentEmptyException && emptyRetry < 1){
                        AppLog.put("播放内容为空，自动重新请求一次")
                        removeLoading(index)
                        Coroutine.async {
                            kotlinx.coroutines.delay(150)
                            if(durChapterIndex == index) loadPlayUrl(emptyRetry + 1)
    ...
    private fun upPlayUrl(){if(forcePlayFromStart){forcePlayFromStart=false;playNew()}else if(isPlayToEnd())playNew()else play()}
    ...
    fun skipTo(index:Int){Coroutine.async{stopPlay();if(index in 0..<simulatedChapterSize){durChapterIndex=index;durChapterPos=0;durPlayUrl="";durLyric=null;forcePlayFromStart=true;saveRead();loadPlayUrl();MaxAudioSystem.syncCurrentBook(book)}}}
    fun prev(){Coroutine.async{stopPlay();if(durChapterIndex>0){durChapterIndex--;durChapterPos=0;durPlayUrl="";durLyric=null;forcePlayFromStart=true;saveRead();loadPlayUrl();MaxAudioSystem.syncCurrentBook(book)}}}
    fun next(){stopPlay();upReadTime();when(playMode){PlayMode.LIST_END_STOP->if(durChapterIndex+1<simulatedChapterSize){durChapterIndex++;durChapterPos=0;durPlayUrl="";durLyric=null;forcePlayFromStart=true;saveRead();loadPlayUrl()};PlayMode.SINGLE_LOOP->{durChapterPos=0;durPlayUrl="";durLyric=null;forcePlayFromStart=true;saveRead();loadPlayUrl()};PlayMode.RANDOM->{if(simulatedChapterSize>0){durChapterIndex=(0 until simulatedChapterSize).random();durChapterPos=0;durPlayUrl="";durLyric=null;forcePlayFromStart=true;saveRead();loadPlayUrl()}};PlayMode.LIST_LOOP->{if(simulatedChapterSize>0){durChapterIndex=(durChapterIndex+1)%simulatedChapterSize;durChapterPos=0;durPlayUrl="";durLyric=null;forcePlayFromStart=true;saveRead();loadPlayUrl()}}};MaxAudioSystem.syncCurrentBook(book)}
