...
object AudioPlay : CoroutineScope by MainScope() {
    ...
    enum class PlayMode(val iconRes: Int) { LIST_END_STOP(R.drawable.ic_play_mode_list_end_stop), SINGLE_LOOP(R.drawable.ic_play_mode_single_loop), RANDOM(R.drawable.ic_play_mode_random), LIST_LOOP(R.drawable.ic_play_mode_list_loop); fun next(): PlayMode = when(this){LIST_END_STOP->SINGLE_LOOP;SINGLE_LOOP->RANDOM;RANDOM->LIST_LOOP;LIST_LOOP->LIST_END_STOP} }
    ...
    fun pause(context:Context){if(AudioPlayService.isRun)context.startService<AudioPlayService>{action=IntentAction.pause}}
    ...
    fun skipTo(index:Int){Coroutine.async{stopPlay();if(index in 0..<simulatedChapterSize){durChapterIndex=index;durChapterPos=0;durPlayUrl="";durLyric=null;saveRead();loadPlayUrl();MaxAudioSystem.syncCurrentBook(book);AudioPlayService.refreshMediaSession()}}}
    fun prev(){Coroutine.async{stopPlay();if(durChapterIndex>0){durChapterIndex--;durChapterPos=0;durPlayUrl="";durLyric=null;saveRead();loadPlayUrl();MaxAudioSystem.syncCurrentBook(book);AudioPlayService.refreshMediaSession()}}}
    fun next(){stopPlay();upReadTime();when(playMode){PlayMode.LIST_END_STOP->if(durChapterIndex+1<simulatedChapterSize){durChapterIndex++;durChapterPos=0;durPlayUrl="";durLyric=null;saveRead();loadPlayUrl()};PlayMode.SINGLE_LOOP->{durChapterPos=0;durPlayUrl="";durLyric=null;saveRead();loadPlayUrl()};PlayMode.RANDOM->{if(simulatedChapterSize>0){durChapterIndex=(0 until simulatedChapterSize).random();durChapterPos=0;durPlayUrl="";durLyric=null;saveRead();loadPlayUrl()}};PlayMode.LIST_LOOP->{if(simulatedChapterSize>0){durChapterIndex=(durChapterIndex+1)%simulatedChapterSize;durChapterPos=0;durPlayUrl="";durLyric=null;saveRead();loadPlayUrl()}}};MaxAudioSystem.syncCurrentBook(book);AudioPlayService.refreshMediaSession()}
    ...
    fun saveRead(first:Boolean=false){saveReadInternal(first,false)}
    fun forceSaveRead(first:Boolean=false){saveReadInternal(first,true)}
    private fun saveReadInternal(first:Boolean,force:Boolean){val currentBook=book?:return;val save={currentBook.lastCheckCount=0;val durTime=System.currentTimeMillis();currentBook.durChapterTime=durTime;val chapterChanged=currentBook.durChapterIndex!=durChapterIndex;currentBook.durChapterIndex=durChapterIndex;currentBook.durChapterPos=durChapterPos;if(first||chapterChanged)appDb.bookChapterDao.getChapter(currentBook.bookUrl,currentBook.durChapterIndex)?.let{currentBook.durChapterTitle=it.getDisplayTitle(ContentProcessor.get(currentBook.name,currentBook.origin).getTitleReplaceRules(),currentBook.getUseReplaceRule(),replaceBook=currentBook.toReplaceBook());SourceCallBack.callBackBook(SourceCallBack.SAVE_READ,bookSource,currentBook,it,durTime.toString())};currentBook.update()};if(force){executor.execute(save)}else{Coroutine.async{save()}}}
