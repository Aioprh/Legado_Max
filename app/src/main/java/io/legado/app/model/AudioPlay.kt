...
object AudioPlay : CoroutineScope by MainScope() {
    ...
    enum class PlayMode(val iconRes: Int) {
        LIST_END_STOP(R.drawable.ic_play_mode_list_end_stop), SINGLE_LOOP(R.drawable.ic_play_mode_single_loop), RANDOM(R.drawable.ic_play_mode_random), LIST_LOOP(R.drawable.ic_play_mode_list_loop);
        fun next(): PlayMode = when (this) { LIST_END_STOP -> SINGLE_LOOP; SINGLE_LOOP -> RANDOM; RANDOM -> LIST_LOOP; LIST_LOOP -> LIST_END_STOP }
    }
    ...
    fun upData(book: Book) {
        AudioPlay.book=book; chapterSize=appDb.bookChapterDao.getChapterCount(book.bookUrl); simulatedChapterSize=if(book.readSimulating()) book.simulatedTotalChapterNum() else chapterSize
        if(durChapterIndex!=book.durChapterIndex){ stopPlay(); durChapterIndex=book.durChapterIndex; durChapterPos=book.durChapterPos; durPlayUrl=""; durLyric=null; durAudioSize=0 }
        upDurChapter(); MaxAudioSystem.syncCurrentBook(book)
    }
    fun resetData(book: Book) {
        stop(); AudioPlay.book=book; readRecord.bookName=book.name; readRecord.bookAuthor=book.author; readRecord.deviceId=AppConst.androidId; readRecord.lastRead=System.currentTimeMillis(); sessionStartTime=System.currentTimeMillis(); readStartTime=System.currentTimeMillis(); lastProgressSaveTime=0L
        chapterSize=appDb.bookChapterDao.getChapterCount(book.bookUrl); simulatedChapterSize=if(book.readSimulating()) book.simulatedTotalChapterNum() else chapterSize; bookSource=book.getBookSource(); durChapterIndex=book.durChapterIndex; durChapterPos=book.durChapterPos
        PlayMode.entries.getOrNull(book.getPlayMode())?.let{playMode=it; postEvent(EventBus.PLAY_MODE_CHANGED,it)}; val speed=book.getPlaySpeed(); AudioPlayService.playSpeed=speed; postEvent(EventBus.AUDIO_SPEED,speed)
        durPlayUrl=""; durLyric=null; durAudioSize=0; synchronized(preloadedUrls){preloadedUrls.clear()}; upDurChapter(); SourceCallBack.callBackBook(SourceCallBack.START_READ,bookSource,book,durChapter); postEvent(EventBus.AUDIO_BUFFER_PROGRESS,0); MaxAudioSystem.syncCurrentBook(book); preloadNextChapters(2)
    }
    fun upReadTime(){if(!AppConfig.enableReadRecord)return; executor.execute{val now=System.currentTimeMillis();readRecord.readTime+=now-readStartTime;readStartTime=now;readRecord.lastRead=now;readRecord.durChapterTitle=book?.durChapterTitle.orEmpty();val session=ReadRecordSession(readRecord.deviceId,readRecord.bookName,readRecord.bookAuthor,sessionStartTime,now,0,readRecord.durChapterTitle);val repository=ReadRecordRepository(appDb.readRecordDao);try{kotlinx.coroutines.runBlocking{repository.saveReadSession(session)}}catch(_:Exception){kotlinx.coroutines.runBlocking{appDb.readRecordDao.insert(readRecord)}};sessionStartTime=now}}
    ...
    fun next(){stopPlay();upReadTime();when(playMode){PlayMode.LIST_END_STOP->if(durChapterIndex+1<simulatedChapterSize){durChapterIndex++;durChapterPos=0;durPlayUrl="";durLyric=null;saveRead();loadPlayUrl()};PlayMode.SINGLE_LOOP->{durChapterPos=0;durPlayUrl="";durLyric=null;saveRead();loadPlayUrl()};PlayMode.RANDOM->{durChapterIndex=(0 until simulatedChapterSize).random();durChapterPos=0;durPlayUrl="";durLyric=null;saveRead();loadPlayUrl()};PlayMode.LIST_LOOP->{if(simulatedChapterSize>0){durChapterIndex=(durChapterIndex+1)%simulatedChapterSize;durChapterPos=0;durPlayUrl="";durLyric=null;saveRead();loadPlayUrl()}}};MaxAudioSystem.syncCurrentBook(book)}
