...
object AudioPlay : CoroutineScope by MainScope() {
    ...
    var chapterTimerCount: Int = 0; private set
    ...
    fun upData(book: Book) {
        val changedBook = this.book?.bookUrl != book.bookUrl
        AudioPlay.book = book
    ...
    fun resetData(book: Book) {
        ...
        val saved = MaxAudioSession.restore(); val restoreSession = saved.currentBookUrl == book.bookUrl && saved.currentIndex in 0 until simulatedChapterSize
        if (restoreSession) { durChapterIndex = saved.currentIndex; durChapterPos = saved.position.coerceAtLeast(0) } else { durChapterIndex = book.durChapterIndex.coerceIn(0, (simulatedChapterSize - 1).coerceAtLeast(0)); durChapterPos = book.durChapterPos.coerceAtLeast(0) }
        book.durChapterIndex = durChapterIndex; book.durChapterPos = durChapterPos
        PlayMode.entries.getOrNull(book.getPlayMode())?.let { playMode = it; postEvent(EventBus.PLAY_MODE_CHANGED, it) }
        val speed = book.getPlaySpeed(); AudioPlayService.playSpeed = if (restoreSession) saved.speed.coerceIn(0.25f, 4f) else speed.coerceIn(0.25f, 4f); postEvent(EventBus.AUDIO_SPEED, AudioPlayService.playSpeed)
        durPlayUrl = ""; durLyric = null; durAudioSize = 0; synchronized(preloadedUrls) { preloadedUrls.clear() }
        upDurChapter(); postEvent(EventBus.AUDIO_PROGRESS, durChapterPos); SourceCallBack.callBackBook(SourceCallBack.START_READ, bookSource, book, durChapter)
        postEvent(EventBus.AUDIO_BUFFER_PROGRESS, 0); MaxAudioSystem.syncCurrentBook(book); AudioPlayService.refreshMediaSession(); preloadNextChapters(2)
    }
    ...
    private fun preloadChapter(currentBook: Book, source: BookSource, index: Int) {
        val key = "${currentBook.bookUrl}#$index"; synchronized(preloadedUrls) { if (preloadedUrls.containsKey(key)) return }; if (!addLoading(index)) return
        val chapter = appDb.bookChapterDao.getChapter(currentBook.bookUrl, index); if (chapter == null || chapter.isVolume) { removeLoading(index); return }
    ...
    fun next() { stopPlay(); upReadTime(); when (playMode) {
        ...
        PlayMode.LIST_LOOP -> if (simulatedChapterSize > 0) { durChapterIndex = (durChapterIndex + 1) % simulatedChapterSize; durChapterPos = 0; book?.durChapterIndex = durChapterIndex; book?.durChapterPos = 0; durPlayUrl = ""; durLyric = null; upDurChapter(); AudioPlayService.refreshMediaSession(); saveRead(); loadPlayUrl() }
    }; MaxAudioSystem.syncCurrentBook(book) }
    ...
    private fun isPlayToEnd(): Boolean = durChapterIndex + 1 == simulatedChapterSize && durAudioSize > 0 && durChapterPos >= durAudioSize
