...
object SmartTag {
    ...
    private val rules = listOf(
        Rule("有声", 100, "书籍类型为音频") { it.type == BookType.audio },
        Rule("漫画", 100, "书籍类型为图片/漫画") { it.isImage },
        Rule("本地", 90, "本地书籍") { it.type == BookType.local },
        Rule("网络书", 60, "来自网络书源") { it.type != BookType.local },
        ...
        Rule("短篇", 50, "章节数少于 50 章") {
            it.totalChapterNum in 1..49
        },
        Rule("有更新", 85, "最近一次检查发现了新章节") { it.lastCheckCount > 0 },
        Rule("更新异常", 90, "最近一次检查出现异常状态") { it.lastCheckCount < 0 },
        Rule("已自定义", 40, "设置了自定义书名分类标签") { !it.customTag.isNullOrBlank() },
