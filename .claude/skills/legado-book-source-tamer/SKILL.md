...
# 本仓库适配说明（移植自 DandanLLab/legadoSkill，MIT 协议）

本技能包已从原项目移植适配到本仓库（阅读Max / legado_Plus，即 Legado 阅读的 fork），使用前请先理解以下适配点：

1. **位置**：本技能包位于 `.claude/skills/legado-book-source-tamer/`，其 `assets/`、`config/`、`docs/`、`debugger/` 都是相对本目录的子目录，引用方式与原项目一致，无需改动。
2. **源码引用**：原文档中所有 `app/src/main/java/io/legado/app/` 路径，在本仓库中即为 `app/src/main/java/io/legado/app/`——本仓库本身就是 Legado 真实源码，调试书源时直接阅读 `app/` 目录下的 Kotlin 源码确认规则实现。
3. **书源输出目录**：原文档中"保存到项目根目录"在本仓库均指本技能包下的 `output/` 目录（即 `.claude/skills/legado-book-source-tamer/output/`）。生成的书源 JSON 与调试产物统一输出到该目录，避免污染 Android 工程根目录。
4. **模拟调试引擎（debugger/）**：仅作参考近似实现，真正权威仍以 `app/` 下源码和 Legado App 实测为准（见下方核心原则）。
...
2. **官方源码是唯一权威** - 本仓库 `app/` 目录下的Kotlin源代码才是真正的规则定义
...
```
模拟失败 → 立即阅读 app/ 目录下的Kotlin源码 → 找到对应规则实现 → 确认正确行为
                                    ↓
...
| 功能 | 源码位置 |
|------|----------|
| CSS选择器 | `app/src/main/java/io/legado/app/model/analyzeRule/` |
| 书源规则 | `app/src/main/java/io/legado/app/data/entities/BookSource.kt` |
| 搜索规则 | `app/src/main/java/io/legado/app/model/SearchBook.kt` |
| 目录规则 | `app/src/main/java/io/legado/app/model/BookChapterList.kt` |
| 正文规则 | `app/src/main/java/io/legado/app/model/WebBook.kt` |
| JS扩展方法 | `app/src/main/java/io/legado/app/help/http/` |
...
4. **app/ 源码目录**
   - 本仓库即Legado完整源码（fork），用于深度理解Legado内部实现
   - 权威参考：`app/src/main/java/io/legado/app/`
...
- 文件名格式：`{书源名称}.json`
- 保存路径：`output/{书源名称}.json`（即本技能包 output 目录）
